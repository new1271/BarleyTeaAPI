...
public final class NBTTagCompoundHelper {
    ...
    @Nonnull
    public static DataComponentMap merge(@Nonnull DataComponentMap mergee, @Nonnull DataComponentMap merger) {
        var builder = DataComponentMap.builder()
                .addAll(mergee)
                .addAll(merger);
        CustomData dataForMergee = mergee.get(DataComponents.CUSTOM_DATA);
        CustomData dataForMerger = merger.get(DataComponents.CUSTOM_DATA);
        CustomData resultData;
        if (dataForMergee != null) {
            if (dataForMerger != null) {
                resultData = CustomData.of(merge(dataForMergee.copyTag(), dataForMerger.copyTag()));
            } else {
                resultData = dataForMergee;
            }
        } else {
            resultData = dataForMerger;
        }
        if (resultData != null) {
            builder = builder.set(DataComponents.CUSTOM_DATA, resultData);
        }
